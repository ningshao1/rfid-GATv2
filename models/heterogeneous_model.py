...
class HeterogeneousGNNModel(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict):
        ...
        for node_type in x_dict:
            if self.res_fc1 is not None and node_type in x_dict_input:
                ...
                x_dict[node_type] = F.elu(
                    x_dict[node_type] + x_dict_input[node_type])
            else:
        ...
        for node_type in x_dict:
            if self.res_fc2 is not None and node_type in x_dict_mid:
                ...
                x_dict[node_type] = F.elu(
                    x_dict[node_type] + x_dict_mid[node_type])
            else:
                x_dict[node_type] = F.elu(x_dict[node_type])
    ...
    def create_heterogeneous_data(self, features_norm, labels_norm, k=7, pos=None, device='cpu'):
        """
        从普通特征和标签创建异构图数据
        注意：此方法应该在RFIDLocalization类中实现，这里仅提供框架
        """
        ...
        # 这里应当实现从原始数据创建异构图的逻辑
        # 这个方法仅作为参考，需要在RFIDLocalization类中实现完整逻辑
