...
def create_heterogeneous_graph_data(features_norm, labels_norm, antenna_positions, k=7, device='cpu'):
    """
    创建异构图数据，包含标签节点和天线节点
    ...
    if not isinstance(antenna_positions, torch.Tensor):
        antenna_positions = torch.tensor(
            antenna_positions, dtype=torch.float32)
    ...
    # 为天线节点创建特征
    antenna_features = torch.full((num_antennas, features_norm.shape[1]), -1.0,
    ...
    edge_index = torch.tensor(
        np.vstack([adj_matrix_coo.row, adj_matrix_coo.col]),
        dtype=torch.long
    ).to(device)
    edge_attr = torch.tensor(
        adj_matrix_coo.data, dtype=torch.float32).view(-1, 1).to(device)
    ...
    for tag_idx in range(num_tags):
        for antenna_idx in range(num_antennas):
            # 计算标签节点到天线的欧氏距离
            pos_dist = torch.sqrt(torch.sum(
                (labels_norm[tag_idx] - antenna_positions[antenna_idx])**2
            ))
    ...
    if tag_to_antenna_edges:
        tag_to_antenna_edge_index = torch.tensor(
            tag_to_antenna_edges, dtype=torch.long).t().to(device)
        antenna_to_tag_edge_index = torch.tensor(
            antenna_to_tag_edges, dtype=torch.long).t().to(device)

        tag_to_antenna_edge_attr = torch.tensor(
            tag_to_antenna_attrs, dtype=torch.float32).view(-1, 1).to(device)
        antenna_to_tag_edge_attr = torch.tensor(
            antenna_to_tag_attrs, dtype=torch.float32).view(-1, 1).to(device)
...
def add_new_node_to_hetero_graph(hetero_data, new_features, initial_position, k=5, device='cpu'):
    """
    向已有的异构图中添加新节点
    ...
    for tag_idx in range(len(all_tag_features)):
        for antenna_idx in range(num_antennas):
            # 计算标签节点到天线的欧氏距离
            pos_dist = torch.sqrt(torch.sum(
                (all_tag_positions[tag_idx] -
                 antenna_positions[antenna_idx])**2
            ))
